...
The Denny's dataset has `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each one of these rows represents a single Denny's establishment. The variables consist of the address, city, state, zip code, longiude, and latitude of each establishment. 

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
The Laquinta dataset has `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each one of these rows represents a single La Quinta establishment. The variables are: `r names(laquinta)`
...
There are La Quinta's in other coutnries besides the US. Denny's, on the other hand, only has locations in within the US (based on the dataset)
...
We could compare each establishment dataset to the states dataset. 
...
Denny's has 403 locations in California (most) and only 1 in Delaware (least). La Quinta has 237 locations in Texas (most) and only 1 in Maine. I do not think this is surprising because I cannot think of a reason as to why it would be surprising in the first place. 
...
dennys %>%
  count(state) %>%
  inner_join(states, by = c("state" = "abbreviation")) %>%
  mutate(perthousand = n/area*1000) %>%
...
District of Columbia has the most Denny's per thousand square miles with 29 locations per. Rhode Island has the most La Quintas per thousand square miles with 1.3 per. 
...
In some cases it does hold up, but the large majority of Denny's locations do not have a La Quinta next to them. 
...
The joke makes a lot more sense for Texas than it does for North Carolina! It looks like every single Denny's has a La Quinta next to it, or at least within a very close proximity. 
